...
import { rapaportsActions as actions } from '.';
...
/** * APIs */
const API_PATH = 'rapaport';
const MODULE_ID = 'seq_no';
const API_ONE_PATH = 'rapaports';
...
const GET_DET_URL = `/api/v1/${API_ONE_PATH}`;
...
function getDetApi(data: { QueryParams: any }) {
	if (data && data.QueryParams) {
		return request({
...
function* getDet(data: KeyedObject) {
	try {
		const { payload } = data;

		const res: IResponse = yield call(getDetApi, payload);
...
export function* Saga() {
	...
	yield takeLatest(actions.getDet.type, getDet);
}
