...
import { CheckBoxEditor, TextFieldEditor } from '@components/table/editors'; // , AutocompleteEditor
import { zodResolver } from '@hookform/resolvers/zod';
import { FormSchema, IFormInput } from '@pages/master/labs/models/Labs';
...
const Labs = ({ handleDrawerToggle, edit, selectedData }: Props) => {
	// add your Dispatch 👿
	...
	// *** Labs State *** //
	const labsState = useSelector(labsSelector);
	const { addError, addSuccess, getSuccess } = labsState;
	...
	useEffect(() => {
		...
		dispatch(labsActions.get({ QueryParams: queryStirng }));
		return () => {
	...
	const handleSubmit = async (event: any) => {
		...
		if (Object.keys(pageState.values).length === 0) {
			...
			if (errorMessage == null) {
				dispatch(labsActions.add(pageState.values));
			} else {
				dispatch(
					actions.openSnackbar({
						open: true,
