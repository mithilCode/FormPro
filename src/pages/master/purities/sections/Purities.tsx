...
import { FormSchema, IFormInput } from '@pages/master/purities/models/Purities';
...
const Purities = () => {
	...
	const { addError, addSuccess, getSuccess } = puritiesState;
	...
	useEffect(() => {
		...
		dispatch(puritiesActions.get({ QueryParams: queryStirng }));
	...
	const handleSubmit = async (event: any) => {
		...
		if (Object.keys(pageState.values).length === 0) {
			...
			if (errorMessage == null) {
				dispatch(puritiesActions.add(pageState.values));
			} else {
				dispatch(
					actions.openSnackbar({
						open: true,
