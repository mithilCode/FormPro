...
import { FormSchema, IFormInput } from '@pages/master/polishs/models/Polishs';
...
const Polishs = () => {
	...
	const { addError, addSuccess, getSuccess } = polishsState;
	...
	useEffect(() => {
		...
		dispatch(polishsActions.get({ QueryParams: queryStirng }));
		return () => {
	...
	const handleSubmit = async (event: any) => {
		...
		if (Object.keys(pageState.values).length === 0) {
			...
			if (errorMessage == null) {
				dispatch(polishsActions.add(pageState.values));
			} else {
				dispatch(
					actions.openSnackbar({
						open: true,
