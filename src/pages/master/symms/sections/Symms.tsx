...
import { FormSchema, IFormInput } from '@pages/master/symms/models/Symms';
...
const Symms = () => {
	...
	const { addError, addSuccess, getSuccess } = symmsState;
	...
	useEffect(() => {
		...
		dispatch(symmsActions.get({ QueryParams: queryStirng }));
		return () => {
	...
	const handleSubmit = async (event: any) => {
		...
		if (Object.keys(pageState.values).length === 0) {
			...
			if (errorMessage == null) {
				dispatch(symmsActions.add(pageState.values));
			} else {
				dispatch(
					actions.openSnackbar({
						open: true,
