...
import { paraRuleActions as actions } from '.';
...
const GET_ONE_PARA_RULE_VIEW_URL = `/api/v1/${API_PATH}`;
const GET_ONE_PARA_RULE_FILE_URL = `/api/v1/${API_PATH}`;
...
function getOneParaRuleViewApi(data: KeyedObject) {
	return request({ url: `${GET_ONE_PARA_RULE_VIEW_URL}/${data}`, method: 'GET' });
}

function getOneParaRuleFileApi(data: KeyedObject) {
	return request({ url: `${GET_ONE_PARA_RULE_FILE_URL}/${data}`, method: 'GET' });
}
...
function* getOneParaRuleView(data: KeyedObject) {
	try {
		const { payload } = data;

		const res: IResponse = yield call(getOneParaRuleViewApi, payload);

		if (res.data) {
			yield put(actions.getOneParaRuleViewSuccess(res.data));
		} else {
			yield put(actions.getOneParaRuleViewError(res.data));
		}
	} catch (e: any) {
		if (e.data) {
			yield put(
				actions.getOneParaRuleViewError({
					error: e.data
				})
			);
		}
...
function* getOneParaRuleFile(data: KeyedObject) {
	try {
		const { payload } = data;

		const res: IResponse = yield call(getOneParaRuleFileApi, payload);

		if (res.data) {
			yield put(actions.getOneParaRuleFileSuccess(res.data));
		} else {
			yield put(actions.getOneParaRuleFileError(res.data));
		}
	} catch (e: any) {
		if (e.data) {
			yield put(
				actions.getOneParaRuleFileError({
					error: e.data
				})
			);
		}
...
export function* Saga() {
	...
	yield takeLatest(actions.getOneParaRuleView.type, getOneParaRuleView);
	yield takeLatest(actions.getOneParaRuleFile.type, getOneParaRuleFile);
