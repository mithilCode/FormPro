...
import { FormSchema, IFormInput } from '@pages/master/colors/models/Colors';
...
const Colors = () => {
	...
	// *** Colors State *** //
	const colorsState = useSelector(colorsSelector);
	const { addError, addSuccess, getSuccess } = colorsState;
	...
	useEffect(() => {
		...
		dispatch(colorsActions.get({ QueryParams: queryStirng }));
		return () => {
	...
	const handleSubmit = async (event: any) => {
		...
		if (Object.keys(pageState.values).length === 0) {
			...
			for (let i in pageState.values.para) {
				if (pageState.values.para[i].action != 'delete') {
					if (
						...
						pageState.values.para[i].full_name == null ||
						pageState.values.para[i].full_name == ''
					) {
						errorMessage = 'Full Name must be required.';
					} else if (
			...
			if (errorMessage == null) {
				dispatch(colorsActions.add(pageState.values));
			} else {
				dispatch(
					actions.openSnackbar({
						open: true,
